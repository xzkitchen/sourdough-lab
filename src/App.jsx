...
import { STEPS_SOURDOUGH, STEPS_TOAST, STEPS_JAPANESE } from './data/steps';
...
// 配方计算函数
// ============================================
function calculateRecipe(breadType, numBreads, flavorType) {
  if (breadType === 'japanese') {
    ...
    const tangzhongNeeded = 150; // 汤种成品用量（预留损耗后）
  ...
  if (breadType === 'toast') {
    ...
    return {
      ...
      allulose: base.allulose * ratio,
      milk: base.milk * ratio,
      tangzhong: base.tangzhong * ratio,
      starter: base.starter * ratio,
      milkPowder: base.milkPowder * ratio,
      salt: base.salt * ratio,
      butterDough: base.butterDough * ratio,
      butterFilling: base.butterFilling * ratio,
      // 汤种原料
      flourTangzhong: 20 * ratio,
      milkTangzhong: 100 * ratio,
      // 汇总
      totalWeight: (230 + 20 + 95 + 110 + 100 + 12 + 5 + 15 + 15) * ratio,
      hydration: 70,
      piecesPerLoaf: 3,
    };
  }
  ...
  return {
    ...
    basil,
    totalWeight,
    perLoaf: Math.round(totalWeight / numBreads),
    hydration: 70,
  };
}
...
// 喂养计算函数
// ============================================
function calculateFeed(recipeNeeds, seedStarter) {
  const buffer = 60; // 余量
  ...
  return {
    ...
    flour,
    water,
    total: seedStarter + flour + water,
    buffer,
  };
}

// ============================================
// 步骤构建函数 - 动态生成 tips 和 ingredients
// ============================================
function buildSteps(breadType, recipe, feed, numBreads, flavorType) {
  const baseSteps = breadType === 'japanese' ? STEPS_JAPANESE :
                    breadType === 'toast' ? STEPS_TOAST : STEPS_SOURDOUGH;
  
  return baseSteps.map(step => {
    let ingredients = [];
    let tips = [...step.tips]; // 复制默认 tips
    ...
    if (breadType === 'japanese') {
      ...
      if (step.id === 'mix') {
        ingredients = [
          ...
          { name: '汤种', value: recipe.tangzhongNeeded, unit: 'g (凉透)' },
    ...
    if (breadType === 'toast') {
      if (step.id === 'tangzhong') {
        ...
        tips = [
          ...
          `【关于损耗】: 原料共 ${rawTotal}g，需用 ${recipe.tangzhong}g。若不足，补牛奶至 ${recipe.tangzhong}g`,
        ];
      }
      if (step.id === 'feed') {
        ingredients = [
          { name: '旧种', value: feed.seed },
          { name: '高粉', value: feed.flour },
      ...
      if (step.id === 'mix') {
        ingredients = [
          ...
          { name: '阿洛酮糖', value: recipe.allulose },
          { name: '冰牛奶', value: recipe.milk },
          { name: '汤种', value: recipe.tangzhong },
          { name: '奶粉', value: recipe.milkPowder },
      ...
      if (step.id === 'knead') {
        ingredients = [
          { name: '海盐', value: recipe.salt },
          { name: '软化黄油', value: recipe.butterDough },
        ];
      }
      if (step.id === 'divide') {
        const totalPieces = numBreads * 3;
      ...
      if (step.id === 'shape') {
        ingredients = [{ name: '有盐黄油', value: recipe.butterFilling, unit: 'g (冷冻)' }];
        tips = [
          '【一次擀卷】: 擀开卷起，松弛10分钟',
          '【二次擀卷】: 再次擀长，压薄底边',
    ...
    if (breadType === 'sourdough') {
      ...
      if (step.id === 'fold_2' && flavorType === 'tomato') {
        ingredients = [
...
function App() {
  ...
  const recipe = useMemo(() => 
    calculateRecipe(breadType, numBreads, flavorType),
    [breadType, numBreads, flavorType]
  );

  // 计算喂养
  const feed = useMemo(() => 
    calculateFeed(recipe.starter, seedStarter),
    [recipe.starter, seedStarter]
  );

  // 构建步骤
  const steps = useMemo(() => 
    buildSteps(breadType, recipe, feed, numBreads, flavorType),
    [breadType, recipe, feed, numBreads, flavorType]
  );
  ...
  return (
    <div className="min-h-screen relative overflow-hidden pb-20">
      ...
      <div className="fixed inset-0 pointer-events-none">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] from-stone-900/40 via-[#0c0a09] to-[#0c0a09]" />
        <div className="absolute top-[-15%] left-[-10%] w-[600px] h-[600px] bg-orange-800/20 rounded-full blur-[120px] mix-blend-screen" />
        <div className="absolute top-[10%] right-[-20%] w-[500px] h-[500px] bg-rose-900/20 rounded-full blur-[100px] mix-blend-screen" />
        <div className="absolute bottom-[-10%] left-[20%] w-[600px] h-[600px] bg-amber-900/10 rounded-full blur-[120px] mix-blend-screen" />
      </div>
        ...
        {activeTab === 'recipe' && (
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-700">
            ...
            <div className="grid grid-cols-3 gap-2 mb-2">
              {[
                { id: 'sourdough', icon: Icons.Bread, title: '乡村酸种', subtitle: 'Sourdough' },
                { id: 'toast', icon: Icons.Toast, title: '奶盐吐司', subtitle: 'Levain' },
                { id: 'japanese', icon: Icons.Toast, title: '日式吐司', subtitle: 'Shokupan' },
              ].map(type => (
                <button
                  key={type.id}
                  onClick={() => handleSwitchBreadType(type.id)}
                ...
                >
                  ...
                  <div className="font-bold text-xs text-white">{type.title}</div>
                  <div className="text-[9px] text-neutral-500 mt-0.5">{type.subtitle}</div>
                </button>
              ))}
            </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  >
                    ...
                    <div className={`font-bold text-xs ${flavorType === f.id ? 'text-white' : 'text-neutral-400'}`}>
                      {f.title}
                    </div>
                  </button>
                ))}
            ...
            <div className="bg-neutral-900/60 backdrop-blur-sm rounded-3xl p-6 border border-white/5">
              <StepperControl
                label={
                  breadType === 'japanese' ? '吐司数量 (原料597g/个)' :
                  breadType === 'toast' ? '吐司数量 (原料602g/个)' :
                  '面包数量 (Loaves)'
                }
                value={numBreads}
            ...
            <div className="border border-white/10 rounded-3xl overflow-hidden bg-neutral-900/60 backdrop-blur-sm">
              <div className="px-6 py-5 border-b border-white/5 flex justify-between items-baseline bg-black/20">
                ...
                <span className="text-[10px] font-bold text-neutral-400 bg-white/5 border border-white/5 px-2 py-1 rounded">
          ...
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-6 duration-700">
            {breadType === 'japanese' ? (
              ...
              <>
                ...
                <div>
                  <div className="text-center text-[10px] text-neutral-500 mb-4 uppercase tracking-widest font-bold">
                    1:1 喂养方案
                  </div>
